...
const Navbar: React.FC<NavbarProps> = ({ className }) => {
  ...
  // Handle body class for drawer animation
  useEffect(() => {
    if (isMenuOpen) {
  ...
  return (
    <>
      {/* Desktop drawer */}
      <div className={cn(
        "fixed left-0 top-0 w-[240px] bg-white h-screen z-40 transition-transform duration-300 ease-in-out shadow-lg",
        isMenuOpen ? "translate-x-0" : "translate-x-[-240px]"
      )}>
        <div className="flex flex-col p-8 space-y-8 h-full">
          <div className="space-y-8">
            <Link to="/" className={cn("text-lg font-medium", getActiveClass('/'))}>Home</Link>
            <Link to="/about" className={cn("block text-lg font-medium", getActiveClass('/about'))}>About</Link>
            <Link to="/case-studies" className={cn("block text-lg font-medium", getActiveClass('/case-studies'))}>Work</Link>
          </div>
        </div>
      </div>
      
      {/* Main navbar */}
      <nav className={cn("fixed left-0 top-0 h-screen z-50 bg-white w-[72px] border-r border-gray-100 flex flex-col justify-between py-6 transition-all duration-300 ease-in-out hidden md:flex", 
        isMenuOpen ? "translate-x-[240px]" : "translate-x-0",
        className
      )}>
